...
router.post('/add-patient', (req, res) => {
    const { first_name,
        ...
        const empty_check = first_name != '' && last_name != '' && age != '' && date_of_diagnosis != '' && date_of_surgery != '' && histopath_results != '' && gleason_score != '' && date_of_treatment != '' && treatment_type != '' && ecog_score != '' &&
        ...
        .then(() => {
            ...
            alert(`Patient ${first_name} ${last_name} added successfully. Returning to home`)
            location.href = '/'
        })
        .catch((err) => {
